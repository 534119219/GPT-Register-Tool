...
- `--one-click-sms`: runs Codex OAuth for selected existing accounts, completes SMSBower phone verification, and stores the OAuth refresh token. The desktop `【一键接码】` dialog fixes the service to OpenAI (`service=dr`), loads every currently available country and price tier from SMSBower `getCountries` plus `getPricesV2`, and remembers the last selected country/tier. Batch one-click SMS forces one phone per email account and prints the successful email→phone mapping in the JSON result.
- Desktop phone verification is SMSBower-only. The settings page keeps credentials, timeout, retry, and OAuth controls; country/tier selection lives in `【一键接码】`, and the old static phone-pool editor is no longer exposed. The CLI retains `--phone-source phone_pool` only as a compatibility seam for existing automation. Outside one-click SMS, one acquired activation is reused up to `phone_reuse.max_reuse_count` times, default `1`. `phone_reuse.send_retry_attempts` handles recoverable add-phone rate limits without immediately canceling the activation, while `number_attempts` controls same-run replacement for rejected or silent numbers.
- Sidebar actions that require an email selection share one themed `【未选择邮箱】` dialog. Selection lookup and deduplication are centralized instead of being reimplemented by payment, inbox, deletion, and SMS handlers.
...
CFWorker mailboxes are polled through the configured Worker endpoint. The desktop inbox uses the same backend adapter as registration, so encoded subjects and MIME message bodies are decoded before display. OTP extraction accepts both `Your temporary ChatGPT verification code` and `Your temporary ChatGPT login code`, and `email_registration.cfworker_otp_issued_after_grace_seconds` gives the provider a small timestamp grace window so a message received 1-10 seconds before the local resend-return time is still considered fresh.
